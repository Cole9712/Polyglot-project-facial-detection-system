...
POINTS = [RIGHT_EYE_POINTS, LEFT_EYE_POINTS, LEFT_BROW_POINTS,
...
def face_swap(img1, img2):
    ...
    img1 = cv2.resize(
        img1, (img1.shape[1] * SCALE_FACTOR, img1.shape[0] * SCALE_FACTOR))
    ...
    img2 = cv2.resize(
        img2, (img2.shape[1] * SCALE_FACTOR, img2.shape[0] * SCALE_FACTOR))
    ...
    all_in_one_mask = np.max(
        [get_face_mask(img1, img1LM), warped_mask], axis=0)
...
if __name__ == "__main__":
    ...
    if execType == "detect":
        ...
        try:
            ...
            pic1 = cv2.putText(pic1, str(
                e), (5, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
        cv2.imwrite("facialOutput/" + fileName1, pic1)
    elif execType == "detectAPI":
        fileName = sys.argv[2]
        pic = cv2.imread('homeOutput/' + fileName, cv2.IMREAD_COLOR)
        result = face_detect(pic)
        cv2.imwrite("facialOutput/" + fileName, result)
    else:
