<template>
  <div :class="[uploaderClass]">
    ...
    <div v-if="multiple" ref="multipleUpload" class="upload-list"></div>
      ...
      >
        ...
        >
          <li>
            ...
            ></span>
            <div
              class="qq-progress-bar-container-selector qq-progress-bar-container"
            >
              <div
                role="progressbar"
                aria-valuenow="0"
  ...
  mounted() {
    ...
    if (!this.multiple) {
      ...
      that.options.callbacks.onDelete = (id) => {
        ...
        that.$emit(
          "done",
          that.uploadedFiles.filter((val, index) => {
...
.icon-uploader-open:before {
  content: "\e683";
}
.icon-uploader-done:before {
  content: "\e65a";
}
</style>